...
public class MoveTemplatesWizard : ScriptableWizard
{
    [SerializeField] private List<Object> instanceEditorDirectories = new List<Object>();

    [SerializeField] private Object outputDirectory;

    private List<InstanceEditor> instanceEditors = new List<InstanceEditor>();

    [MenuItem("Tools/SUFanGame/Templates/Move Templates")]
    public static void CreateWizard()
    {
        ScriptableWizard.DisplayWizard<MoveTemplatesWizard>("Move Templates", "Move", "Cancel");
    }
    ...
    void OnWizardCreate()
    {
        ...
        if (Utilities.GetObjectsInScene<InstanceEditor>().Length > 0)
        {
            ToolUtilities.CancelOperation(
                "This tool can not be used while there are InstanceEditors loaded in the scene!");
        }

        //Verify that the user wants to perform this time consuming operation
        if (
            !EditorUtility.DisplayDialog("Move Templates",
                "This tool will move the specificied templates to the given location and refactor all json files accordingly. This could take a very long time. Are you sure you wish to proceed?",
                "Yes", "Cancel"))
        {
        ...
        if (
            !EditorUtility.DisplayDialog("Move Templates",
                "You should not use this tool if there are InstanceEditors loaded in ANY SCENE, not just this one. If you attempt to move templates that have loaded InstanceEditors in other scenes, they will be disconnected and become unsavable. Be sure and save any chunk changes you have made BEFORE using this tool to avoid data loss.",
        ...
        EditorUtility.DisplayProgressBar("Moving Templates", "Initializing...", progress);

        string instanceEditorOutputDirectoryAssetPath = AssetDatabase.GetAssetPath(outputDirectory);

        string templateOutputDirectoryAppDataPath =
            Utilities.ConvertEditorInstanceAssetPathToMirroringTemplateAppDataPath(
                instanceEditorOutputDirectoryAssetPath);

        //Ensure the output directory exists
        string templateOutputDirectoryAbsolutePath =
            Utilities.ConvertAppDataPathToAbsolutePath(templateOutputDirectoryAppDataPath);
        ...
        foreach (InstanceEditor instanceEditor in instanceEditors)
        {
            ...
            string originalInstanceEditorDirectoryAssetPath =
                Utilities.ConvertPathToParentDirectoryPath(AssetDatabase.GetAssetPath(instanceEditor));
            string newInstanceEditorDirectoryAssetPath = instanceEditorOutputDirectoryAssetPath + "/" +
                                                         Utilities.ConvertDirectoryPathToDirectoryName(
                                                             originalInstanceEditorDirectoryAssetPath);
            ...
            string originalTemplateDirectoryAbsolutePath =
                Utilities.ConvertAppDataPathToAbsolutePath(
                    Utilities.ConvertPathToParentDirectoryPath(instanceEditor.Instance.TemplateAppDataPath));
            string newTemplateDirectoryAbsolutePath = templateOutputDirectoryAbsolutePath + "/" +
                                                      Utilities.ConvertDirectoryPathToDirectoryName(
                                                          originalTemplateDirectoryAbsolutePath);
            ...
            string newTemplateAppDataPath =
                Utilities.ConvertAbsolutePathToAppDataPath(newTemplateDirectoryAbsolutePath) + "/" + instanceEditor.name;
            instanceEditor.Instance.TemplateAppDataPath = newTemplateAppDataPath;
        }

        //Update the Chunk JSON files to match the movement
        string[] chunkAbsolutePaths = Directory.GetFiles(Utilities.ExternalDataPath + "/Chunks", "*.json",
        ...
        foreach (string chunkAbsolutePath in chunkAbsolutePaths)
        {
            ...
            //Display the progress bar
            EditorUtility.DisplayProgressBar("Moving Templates", string.Format("Processing '{0}'...", currentChunkName),
                progress);
            ...
            foreach (InstanceEditor instanceEditor in instanceEditors)
            {
                ...
                string newTemplateAppDataPath = string.Format("\"{0}/{1}/{2}\"", templateOutputDirectoryAppDataPath,
                    instanceEditor.name, instanceEditor.name);
        ...
        if (instanceEditorDirectories.Count > 0)
        {
            ...
            foreach (Object instanceEditorDirectory in instanceEditorDirectories)
            {
                ...
                else
                {
                    ...
                    string expectedInstanceEditorAbsolutePath = string.Format("{0}/{1}.prefab",
                        instanceEditorAbsolutePath, instanceEditorName);
                    string expectedInstanceEditorAssetPath =
                        Utilities.ConvertAbsolutePathToAssetPath(expectedInstanceEditorAbsolutePath);
                    GameObject expectedInstanceEditorGameObject =
                        AssetDatabase.LoadAssetAtPath<GameObject>(expectedInstanceEditorAssetPath);
                    ...
                    {
                        InstanceEditor expectedInstanceEditor =
                            expectedInstanceEditorGameObject.GetComponent<InstanceEditor>();
